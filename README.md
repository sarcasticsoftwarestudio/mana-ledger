...
- **Briefing** — a searchable, first-class reader for official Wizards announcements, release notes, rules updates, and product features. It keeps a local article archive, tracks new/unread/updated and saved articles, detects source changes, supports date/author/set/card filters plus search within an article, and exposes parser diagnostics. Parsed galleries are collection-aware, include a zoomable source-image viewer, add missing cards to the want list, and support persistent manual Scryfall corrections when an automatic match needs help.
