...
- **Briefing** — a first-class reader for official Wizards announcements, release notes, rules updates, and product features. It keeps a local article archive, accepts any Wizards news URL, extracts useful sections and PDFs across varying layouts, and turns detected set ranges/card notes into an exact-printing image gallery with the same hover details and card modal used elsewhere in the app.
